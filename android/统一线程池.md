...
```java
DispatcherExecutor.getIOExecutor().execute(new Runnable());
...
public class DispatcherExecutor {
  ...
  /**
   * CPU 密集型任务的线程池
   * 核心池大小和 CPU 核心数相关。常见的 CPU 密集型任务如比较复杂的计算操作，此时需要使用大量的 CPU 计算单元。
   */
  private static ThreadPoolExecutor sCPUThreadPoolExecutor;

  /**
   * IO 密集型任务的线程池
   * IO 密集型任务不消耗 CPU，核心池可以很大。常见的 IO 密集型任务如文件读取、写入，网络请求等等。
   */
